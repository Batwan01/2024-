...
num_dec_layer = 6
loss_lambda = 2.0
num_classes = 14 # original 80

image_size = (1024, 1024)
batch_augments = [
    dict(type='BatchFixedSizePad', size=image_size, pad_mask=False) # pad_mask=False
]
model = dict(
    type='CoDETR',
...
load_pipeline = [
    ...
    dict(type='LoadAnnotations', with_bbox=True, with_mask=False), # with_mask=False
    dict(
        type='RandomResize',
        scale=image_size,
        ratio_range=(0.1, 2.0),
...
train_pipeline = [
    dict(type='CopyPaste', max_num_pasted=100, paste_by_box=True), # paste_by_box=True
